...
# FanCI Legacy objective class from FanCI code
class ProjectedSchrodingerLegacyFanCI(metaclass=ABCMeta):
    """
    FanCI problem class.
    ...
    def __init__(
        ...
        wfn = self.fill_wavefunction(wfn, nproj, fill)
    ...
    def add_constraint(self, name: str, f: Callable, dfdx: Callable = None) -> None:
        ...
        """
        self.constraints[name] = f, dfdx

        # Update nequation
        self.nequation = self.nproj + len(self.constraints)
    ...
    def compute_objective(self, x: np.ndarray) -> np.ndarray:
        ...
        f = np.empty(self.nequation, dtype=pyci.c_double)
        f_proj = f[: self.nproj]
        f_cons = f[self.nproj :]
        ...
        self.ci_op(ovlp, out=f_proj)
        ovlp_proj = ovlp[: self.nproj]
        ovlp_proj *= energy
        f_proj -= ovlp_proj
    ...
    def make_param_constraint(self, i: int, val: float) -> Tuple[Callable, Callable]:
        ...
        def f(x: np.ndarray) -> float:
            ...
            """
            return x[i] - val if self.mask[i] else 0
    ...
    def make_det_constraint(self, i: int, val: float) -> Tuple[Callable, Callable]:
        ...
        def dfdx(x: np.ndarray) -> np.ndarray:
            ...
            """
            y = np.zeros(self.nactive, dtype=pyci.c_double)
            d_ovlp = self.compute_overlap_deriv(x[:-1], self.sspace[np.newaxis, i])[0]
            y[: self.nactive - self.mask[-1]] = d_ovlp
            return y
    ...
    @abstractmethod
    def compute_overlap_deriv(self, x: np.ndarray, occs_array: Union[np.ndarray, str]) -> np.ndarray:
        """
...
# Parent Interface Class to add PyCI features to legacy FanCI objective class
class ProjectedSchrodingerFanCI(ProjectedSchrodingerLegacyFanCI):
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        ProjectedSchrodingerLegacyFanCI.__init__(
            self,
            ham,
            wfn,
            nproj,
