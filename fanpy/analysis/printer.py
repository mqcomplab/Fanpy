...
def sds_occ(sds, nspatial):
    ...
    sds_str = [
        [
            format(slater.split_spin(sd, nspatial)[0], f"0{nspatial}b"),
            format(slater.split_spin(sd, nspatial)[1], f"0{nspatial}b"),
        ]
        for sd in sds
    ]
    # Print header
...
def sds_occ_indices(sds, nspatial):
    """Print Slater determinants from a wavefunction as lists of occupied MO indices.
