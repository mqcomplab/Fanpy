...
def test_init():
    """Test FixedEmbeddedWavefunction.assign_wfns."""
    wfn1 = CIWavefunction(2, 6)
    wfn2 = CIWavefunction(2, 6)
    wfn = FixedEmbeddedWavefunction(4, [2, 2], 12, [[0, 2, 4, 6, 8, 10], [1, 3, 5, 7, 9, 11]], [wfn1, wfn2])
    assert wfn.nelec == 4
    assert wfn.nelec_list == [2, 2]
    assert wfn.nspin == 12
    ...
    with pytest.raises(ValueError):
        wfn = FixedEmbeddedWavefunction(4, [2, 2], 12, [[0, 0, 2, 4, 6, 8, 10], [1, 3, 5, 7, 9, 11]], [wfn1, 2])
    with pytest.raises(TypeError):
        wfn = FixedEmbeddedWavefunction(4, [2, 2], 12, [[0, 2, 4, 6, 8, 10], [1, 3, 5, 7, 9, 11]], [wfn1, 2])
    with pytest.raises(ValueError):
        wfn = FixedEmbeddedWavefunction(4, [2, 2], 12, [[0, 2, 4, 6, 8, 10], [1, 3, 5, 7, 9, 11]], [wfn1])
    with pytest.raises(ValueError):
        wfn1.nspin = 7
        wfn = FixedEmbeddedWavefunction(4, [2, 2], 12, [[0, 2, 4, 6, 8, 10], [1, 3, 5, 7, 9, 11]], [wfn1, wfn2])
    with pytest.raises(ValueError):
        wfn1.nelec = 1
        wfn1.nspin = 6
        wfn = FixedEmbeddedWavefunction(
            4, [2, 2], 12, [[0, 2, 4, 6, 8, 10], [1, 3, 5, 7, 9, 11]], [wfn1, wfn2], disjoint=True
        )
    with pytest.raises(ValueError):
        wfn1.nelec = 2
        wfn1.nspin = 5
        wfn = FixedEmbeddedWavefunction(
            4, [2, 2], 12, [[0, 2, 4, 6, 8], [1, 3, 5, 7, 9, 11]], [wfn1, wfn2], disjoint=True
        )
    with pytest.raises(ValueError):
        wfn1.nelec = 2
        wfn1.nspin = 7
        wfn = FixedEmbeddedWavefunction(
            4, [2, 2], 12, [[0, 1, 2, 4, 6, 8, 10], [1, 3, 5, 7, 9, 11]], [wfn1, wfn2], disjoint=True
        )

    wfn1 = CIWavefunction(2, 6)
    wfn = FixedEmbeddedWavefunction(4, [2, 2], 12, [[0, 2, 4, 6, 8, 10], [1, 3, 5, 7, 9, 11]], [wfn1, wfn1])
    assert wfn.nelec == 4
    assert wfn.nelec_list == [2, 2]
    assert wfn.nspin == 12
    ...
    wfn2 = CIWavefunction(2, 6)
    with pytest.raises(ValueError):
        wfn = FixedEmbeddedWavefunction(4, [2, 2], 12, [[0, 1, 2, 3, 4, 6, 8, 10], [1, 3, 5, 7, 9, 11]], [wfn1, wfn2],
                                   disjoint=False)
    with pytest.raises(ValueError):
        wfn = FixedEmbeddedWavefunction(4, [2, 2], 12, [[0, 1, 2, 4, 5, 6, 8, 10], [1, 3, 5, 7, 9, 11]], [wfn1, wfn2],
                                   disjoint=False)
    wfn = FixedEmbeddedWavefunction(4, [2, 2], 12, [[0, 1, 2, 4, 6, 7, 8, 10], [1, 3, 5, 7, 9, 11]], [wfn1, wfn2],
                               disjoint=False)
    assert wfn.nelec == 4
    assert wfn.nelec_list == [2, 2]
    assert wfn.nspin == 12
...
def test_num_systems():
    """Test FixedEmbeddedWavefunction.num_systems."""
    wfn1 = CIWavefunction(2, 6)
    wfn2 = CIWavefunction(2, 6)
    wfn = FixedEmbeddedWavefunction(4, [2, 2], 12, [[0, 2, 4, 6, 8, 10], [1, 3, 5, 7, 9, 11]], [wfn1, wfn2])
    assert wfn.num_systems == 2
    wfn = FixedEmbeddedWavefunction(4, [2, 2], 12, [[0, 2, 4, 6, 8, 10], [1, 3, 5, 7, 9, 11]] * 2,
                               [wfn1, wfn1, wfn1, wfn1], disjoint=False)
    assert wfn.num_systems == 4


def test_split_sd():
    """Test FixedEmbeddedWavefunction.split_sd."""
    wfn1 = CIWavefunction(2, 6)
    wfn2 = CIWavefunction(2, 6)
    wfn = FixedEmbeddedWavefunction(4, [2, 2], 12, [[0, 2, 4, 6, 8, 10], [1, 3, 5, 7, 9, 11]], [wfn1, wfn2])
    ...
    wfn = FixedEmbeddedWavefunction(4, [2, 2], 12, [[0, 1, 2, 4, 6, 7, 8, 10], [1, 3, 5, 7, 9, 11]], [wfn1, wfn2],
                               disjoint=False)
    assert wfn.split_sd(0b000011000011) == [0, 0b001001]
    wfn = FixedEmbeddedWavefunction(4, [4, 2], 12, [[0, 1, 2, 4, 6, 7, 8, 10], [1, 3, 5, 7, 9, 11]], [wfn1, wfn2],
                               disjoint=False)
    assert wfn.split_sd(0b000011000011) == [0b00110011, 0b001001]
    #wfn1 = CIWavefunction(6, 14)
    #wfn2 = CIWavefunction(10, 14)
    #indices = [[0, 1, 2, 3, 4, 5, 6, 14, 15, 16, 17, 18, 19, 20], [7, 8, 9, 10, 11, 12, 13, 21, 22, 23, 24, 25, 26, 27]]
    #wfn = FixedEmbeddedWavefunction(16, [6, 10], 28, indices, [wfn1, wfn2], memory=None, disjoint=True)
    #assert wfn.split_sd(ground(16, 28)) == [ground(6, len(indices[0])), ground(10, len(indices[1]))]


def test_get_overlap():
    """Test FixedEmbeddedWavefunction.get_overlap."""
    wfn1 = CIWavefunction(2, 6)
    wfn1.assign_params(np.random.rand(wfn1.nparams))
    wfn2 = CIWavefunction(2, 6)
    wfn2.assign_params(np.random.rand(wfn2.nparams))

    test = FixedEmbeddedWavefunction(4, [2, 2], 12, [[0, 2, 4, 6, 8, 10], [1, 3, 5, 7, 9, 11]],
    ...
    test = FixedEmbeddedWavefunction(4, [4, 2], 12, [[0, 1, 2, 4, 6, 7, 8, 10], [1, 3, 5, 7, 9, 11]], [wfn1, wfn2],
                                disjoint=False)
    for sd in sd_list(4, 12):
        sd1, sd2 = test.split_sd(sd)
        olp1 = wfn1.params[wfn1.dict_sd_index[sd1]] if sd1 in wfn1.dict_sd_index else 0
...
def test_get_overlaps():
    """Test FixedEmbeddedWavefunction.get_overlaps."""
    wfn1 = CIWavefunction(2, 6)
    wfn1.assign_params(np.random.rand(wfn1.nparams))
    wfn2 = CIWavefunction(2, 6)
    wfn2.assign_params(np.random.rand(wfn2.nparams))
    ...
    wfn2 = CIWavefunction(2, 6)
    wfn2.assign_params(np.random.rand(wfn2.nparams))
    test = FixedEmbeddedWavefunction(4, [2, 2], 12, [[0, 1, 2, 4, 6, 7, 8, 10], [1, 3, 5, 7, 9, 11]], [wfn1, wfn2],
                                disjoint=False)
    for answer, sd in zip(test.get_overlaps(sd_list(4, 12)), sd_list(4, 12)):
        sd1, sd2 = test.split_sd(sd)
        olp1 = wfn1.params[wfn1.dict_sd_index[sd1]] if sd1 in wfn1.dict_sd_index else 0
