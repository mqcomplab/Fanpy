...
def test_init():
    ...
    with pytest.raises(ValueError):
        wfn = FixedEmbeddedWavefunction(
            4, [2, 2], 12, [[0, 1, 2, 3, 4, 6, 8, 10], [1, 3, 5, 7, 9, 11]], [wfn1, wfn2], disjoint=False
        )
    with pytest.raises(ValueError):
        wfn = FixedEmbeddedWavefunction(
            4, [2, 2], 12, [[0, 1, 2, 4, 5, 6, 8, 10], [1, 3, 5, 7, 9, 11]], [wfn1, wfn2], disjoint=False
        )
    wfn = FixedEmbeddedWavefunction(
        4, [2, 2], 12, [[0, 1, 2, 4, 6, 7, 8, 10], [1, 3, 5, 7, 9, 11]], [wfn1, wfn2], disjoint=False
    )
    assert wfn.nelec == 4
    assert wfn.nelec_list == [2, 2]
    assert wfn.nspin == 12
...
def test_num_systems():
    ...
    wfn = FixedEmbeddedWavefunction(
        4, [2, 2], 12, [[0, 2, 4, 6, 8, 10], [1, 3, 5, 7, 9, 11]] * 2, [wfn1, wfn1, wfn1, wfn1], disjoint=False
    )
...
def test_split_sd():
    ...
    wfn = FixedEmbeddedWavefunction(
        4, [4, 2], 12, [[0, 1, 2, 4, 6, 7, 8, 10], [1, 3, 5, 7, 9, 11]], [wfn1, wfn2], disjoint=False
    )
...
def test_get_overlap():
    ...
    test = FixedEmbeddedWavefunction(
        4, [2, 2], 12, [[0, 2, 4, 6, 8, 10], [1, 3, 5, 7, 9, 11]], [wfn1, wfn2], disjoint=False
    )
...
def test_get_overlaps_rbm_ap1rog():
    ...
    test = FixedEmbeddedWavefunction(
        4, [2, 2], 12, [[0, 2, 4, 6, 8, 10], [1, 3, 5, 7, 9, 11]], [ap1rog, rbm], disjoint=False
    )
    sds = sd_list(4, 12)
    assert np.allclose(
